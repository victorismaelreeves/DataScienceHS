...
class KNN:
    ...
    def compute_distances_two_loops(self, X):
        ...
        if self.metric == 'manhattan':
            ...
            for i_test in range(num_test):
                for i_train in range(num_train):
                    # TODO: Fill dists[i_test][i_train]
                    dists[i_test][i_train] = np.sum((self.train_X[i_train] - X[i_test] ** 2))
        return dists          
    ...
    def compute_distances_one_loop(self, X):
        ...
        if self.metric == 'manhattan':
            ...
            for i_test in range(num_test):
                # TODO: Fill the whole row of dists[i_test]
                # without additional loops or list comprehensions
                dists[i_test, :] = np.sum((self.train_X - X[i_test] ** 2), axis=1)
        return dists
    ...
    def compute_distances_no_loops(self, X):
        ...
        if self.metric == 'euclidean':
            dists = np.sqrt(np.sum(X**2, axis=1, keepdims=True) - (2 * np.dot(X, self.train_X.T)) + np.sum(self.train_X**2, axis=1))
        return dists    
    ...
    def predict_labels_binary(self, dists):
        ...
        for i in range(num_test):
            ...
            closest = self.train_y[nearest]
            if self.k/2<np.sum(self.train_y[nearest[:self.k]]):
                pred[i]=True
            else:
                pred[i]=False
        return pred
