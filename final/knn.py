...
class KNN:
    ...
    def compute_distances_two_loops(self, X):
        ...
        if self.metric == 'manhattan':
            ...
            for i_test in range(num_test):
                for i_train in range(num_train):
                    dists[i_test][i_train] = np.sqrt(np.sum(np.square((self.train_X[i_train]) - X[i_test])))
                    pass
    ...
    def compute_distances_one_loop(self, X):
        ...
        if self.metric == 'manhattan':
            ...
            for i_test in range(num_test):
                dists[i_test] = np.sqrt(np.sum(np.square(self.train_X - X[i_test])), axis=1)
                # without additional loops or list comprehensions
                pass
    ...
    def compute_distances_no_loops(self, X):
        ...
        dists = np.zeros((num_test, num_train), np.float32)
        if self.metric == 'manhattan':
            dists = np.abs(self.train_X - X[:, np.newaxis]).sum(-1)
            pass
    ...
    def predict_labels_binary(self, dists):
        ...
        for i in range(num_test):
            ...
            ##closest = []
            closest = np.argpartition(dists[i], self.k)
            if np.true_divide(self.k, 2) < np.sum(self.train_y[closest[:self.k]]):
                pred[i] = True
    ...
    def predict_labels_multiclass(self, dists):
        ...
        for i in range(num_test):
            closest = np.argpartition(dists[i], self.k)
            pred[i] = np.argmax(np.bincount(self.train_y[closest[:self.k]]))
        return pred